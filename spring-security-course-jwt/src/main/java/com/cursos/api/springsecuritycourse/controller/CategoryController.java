...
public class CategoryController {
	...
	public ResponseEntity<Category> createOne(@RequestBody @Valid SaveCategory saveCategory){
		...
		Category category = categoryService.createOne(saveCategory);//mandamos dto saveProduct
		//JPA SI FALLA LANZA EXCEPTION
		return ResponseEntity.status(HttpStatus.CREATED).body(category);//PRODUCTO CREADO
	}
		
	@PutMapping("/{categoryId}")//ESTE METODO HTTP, NECESITA PATH
	//AGREGAMOSUN @RequestBody YA QUE VENDRA EN EL BODY DE LA PETICION
	...
	public ResponseEntity<Category> updateOneById(@PathVariable Long categoryId, @RequestBody @Valid SaveCategory saveCategory){
		//crear el metodo dentro de product service
		Category category = categoryService.updateOneById(categoryId,saveCategory);//mandamos dto saveProduct
		//JPA SI FALLA LANZA EXCEPTION
		return ResponseEntity.ok(category);//CATEGORI CREADA
	}
	
	
	//DISABLE
	@PutMapping("/{categoryId}/disabled")//ESTE METODO HTTP, NECESITA PATH //controlador de tipo restfull (funcion ejecutable que recibe parametros y devuelve info)
	//@PathVariable para identificarlo
	//@Valid PARA VALIDAR PARAMETROS
	public ResponseEntity<Category> disableOneById(@PathVariable Long categoryId){
		//crear el metodo dentro de product service
		Category category = categoryService.disableOneById(categoryId);//mandamos dto saveProduct
		//JPA SI FALLA LANZA EXCEPTION
